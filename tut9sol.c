...
void *Worker(void *);  //This line creates a void type pointer with argument "void". (Dont fully understand)
void InitializeGrids();

/*
This structure construction is a little bit different because it defines the structure in a different way.
In this definition, this structure has been defined, with an alias "mylib_barrier_t". And there is no identifier for this structure definition.
"No identifier" means there is nothing between "struct" and first "{", like the line pointed out below.
*/
typedef struct {    //<------------This line.
...
} mylib_barrier_t;   //This is the alias of this struct. No other classes/symbols can use it.
...
void mylib_barrier(mylib_barrier_t *b, int num_threads) {
  ...
  b -> count ++;  //This line is used to access the member "count" in structure "mylib_barrier_t", using a pointer varialbe "b" to access it.
...
int main(int argc, char *argv[]) {
  ...
  gridSize = atoi(argv[1]);  //atoi() converts the string argument to an integer.
